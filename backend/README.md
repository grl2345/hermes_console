...
- `POST /agents`：等价于 `init_agent.sh` 的 Python 版，**同时自动打好 `hermes.*` labels**

**必填字段**：
```json
{
  "agentName": "test_G02",                    // 容器名与 hermes.id，仅允许字母/数字/_/-
  "telegramBotToken": "7000:AAAbbb...",
...
```json
{
  "displayName": "Alice",          // hermes.name（缺省用 agentName）
  "shortName": "A",                // hermes.shortName（缺省用 displayName 首字）
  "role": "测试 agent",             // hermes.role
  "level": "staff",                // ceo / director / staff，默认 staff
  "reportsTo": "main",             // 上级的 hermes.id
  "avatarColor": "#3B82F6",
  "openaiBaseUrl": "https://openrouter.ai/api/v1",
  "modelBaseUrl": "https://openrouter.ai/api/v1",  // 缺省复用 openaiBaseUrl
  "gatewayAllowAllUsers": true
}
```

**执行的步骤**（与 `init_agent.sh` 对齐）：
1. 在 `HERMES_AGENT_ROOT/hermes_agent_<name>/data/` 下创建目录，写 `.env`（chmod 600 + chown 10000:10000）
2. OpenRouter key 自动检测（key 前缀 `sk-or-` 或 base_url 含 `openrouter.ai`）→ 自动注入 `OPENROUTER_API_KEY`
3. `docker run nousresearch/hermes-agent:latest` 起容器，**带上完整 `hermes.*` labels**
4. 等待容器 running
5. `docker exec -u hermes <container> hermes config set` 写入 `model.default` / `model.provider` / `model.base_url` / `auxiliary.compression.*`
6. `docker restart` 让配置生效

**失败回滚**：任何步骤失败（APIError / 超时 / 文件权限等）都会 stop + rm 容器 + 删除宿主目录，保证不留半成品。

**部署要求**（docker-compose 部署后端时）：
- `HERMES_AGENT_ROOT`（默认 `/root/hermes`）必须挂载进后端容器
- 后端进程需要对该目录有**写权限**（`.env` 文件 chmod 0600 + chown 到 uid 10000）
- 已在 `backend/docker-compose.yml` 自动挂载

**前端提示**：目前 `/dashboard/new-agent` 表单只收集了元信息（name / shortName / role / model），**缺少 secrets 字段**（telegramBotToken / openaiApiKey / openaiBaseUrl），所以直接点"创建"会返 422。要通过 UI 创建需要先扩展表单；当前可以用 curl / Swagger `/docs` 页测试。

### 阶段 9：实时指标

- **CPU / 内存**：后台线程池（由 APScheduler 驱动）每 15s 调 `container.stats(stream=False)`
  并行采样所有 running 的 Hermes agent；结果写进程内缓存（TTL 60s）
- **Agent 列表 / 详情**：从缓存取 cpu / memory 并填进 Agent 响应（首次刷新前为 0）
- **activeTaskCount / todayTaskCount**：从 phase 6 的 `task_store` 实时计算，不从 label 读
- `GET /agents/{id}/stats`：按需快照
  - 缓存新鲜（≤15s）直接返
  - 过期则立即 `sample_now()`
  - 响应体含 `cpu` / `memory` / `memoryLimit` / `weeklyTokens` / `weeklyCost` / `updatedAt`
- `GET /dashboard/stats`：
  - `onlineCount` / `totalAgents`：从 agent 列表
  - `activeTaskCount` / `todayTasks` / `todayCompleted`：从 `task_store`（今日 = 本地时区 0 点后启动）
  - `todayTokens` / `todayCost`：**恒为 0**，等 Hermes agent 自己记账后再接

**暂不做（等数据源）**：
- Token / 费用：Hermes agent 暂未记账，所有相关字段保持 0；UI 显示 "—"
- CPU / 内存历史曲线：当前只有瞬时快照；要画折线需加时序存储（如 SQLite / InfluxDB）

暂不做：数据库持久化、密码哈希、RBAC、Token 记账。
